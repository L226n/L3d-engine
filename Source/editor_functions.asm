f_rotate_normal:
	...
	insert_row	ROTATE_ID, DEFAULT_POINT, 0, 1, 0	;lots of ids here
	insert_row	CENTER_ID, DEFAULT_POINT, 1, 1, 1
	insert_row	VERTEX_1, DEFAULT_POINT, 2, 1, 2	;ids for plane
	insert_row	VERTEX_2, DEFAULT_POINT, 3, 1, 3
	insert_row	VERTEX_3, DEFAULT_POINT, 4, 1, 4
	insert_row	ANGLE, DEFAULT_USTRING, 5, 3, 0
	insert_row	ROTATE_POINT, " ", 6, 0, 0
	add	rax, qword[row_width_editor]	;add row now
	insert_row	DUPLICATE_POINT, DEFAULT_OPT, 7, 4, 0	;and checkboxx
	insert_row	USE_SELECTION, DEFAULT_OPT, 8, 4, 0	;and an option to duplicate points
	mov	byte[update], 1
	ret
.update:
...
f_vector_trig:
	...
	insert_row	FACE_1, DEFAULT_POINT, 0, 1, 0
	insert_row	FACE_2, DEFAULT_POINT, 1, 1, 1
	insert_row	FACE_3, DEFAULT_POINT, 2, 1, 2
	insert_row	ANGLE, DEFAULT_USTRING, 3, 3, 0
	insert_row	APPLY_MOV, " ", 4, 0, 0
	add	rax, qword[row_width_editor]
	insert_row	DUPLICATE_POINT, DEFAULT_OPT, 5, 4, 0	;and checkboxx
	mov	byte[update], 1
	ret
.update:
	mov	r15, qword[points_dat]
...
f_rotate_axis:
	...
	mov	byte[error_space], 16
	insert_row	POINT_ID, DEFAULT_POINT, 0, 1, 0	;3 ids
	insert_row	AXIS_A, DEFAULT_POINT, 1, 1, 1
	insert_row	AXIS_B, DEFAULT_POINT, 2, 1, 2
	insert_row	ANGLE, DEFAULT_USTRING, 3, 3, 0	;angle
	insert_row	ROTATE_POINT, " ", 4, 0, 0	;button
	add	rax, qword[row_width_editor]
	insert_row	DUPLICATE_POINT, DEFAULT_OPT, 5, 4, 0	;and checkboxx
	insert_row	USE_SELECTION, DEFAULT_OPT, 6, 4, 0	;and an option to duplicate points
	mov	byte[update], 1
	ret
.apply:
...
f_move_along_vec:
	...
	mov	byte[error_space], 16
	insert_row	POINT_ID, DEFAULT_POINT, 0, 1, 0	;two idsand a distance
	insert_row	VECTOR_A, DEFAULT_POINT, 1, 1, 1
	insert_row	VECTOR_B, DEFAULT_POINT, 2, 1, 2
	insert_row	MOVE_DIST, DEFAULT_STRING, 3, 2, 0
	insert_row	APPLY_MOV, " ", 4, 0, 0
	add	rax, qword[row_width_editor]
	insert_row	DUPLICATE_POINT, DEFAULT_OPT, 5, 4, 0	;+ a little checkbox
...
.insert_trig:
	...
	sqrtss	xmm1, xmm1	;square root this val
...
f_create_selection:
	...
	insert_row	POINT_ID, DEFAULT_POINT, 0, 1, 0	;id for point to add/sub from selection
	insert_row	SELECTION_ADD, " ", 1, 0, 0	;and 2 buttons
	insert_row	SELECTION_SUB, " ", 2, 0, 0
	insert_row	SELECTION_CLR, " ", 3, 0, 0
...
f_rotate_point:
	...
	insert_row	ROTATE_ID, DEFAULT_POINT, 0, 1, 0	;two ids
	insert_row	CENTER_ID, DEFAULT_POINT, 1, 1, 1
	insert_row	ROTATE_YAW, DEFAULT_USTRING, 2, 3, 0	;three angles
	insert_row	ROTATE_PITCH, DEFAULT_USTRING, 3, 3, 1
	insert_row	ROTATE_ROLL, DEFAULT_USTRING, 4, 3, 2
	insert_row	ROTATE_POINT, " ", 5, 0, 0	;and a button
	add	rax, qword[row_width_editor]	;new row
	insert_row	DUPLICATE_POINT, DEFAULT_OPT, 6, 4, 0	;and an option to duplicate points
	insert_row	USE_SELECTION, DEFAULT_OPT, 7, 4, 0	;and an option to duplicate points
	mov	byte[update], 1
	ret
.update:
	mov	ax, word[option_data_id]	;move in current id
...
f_remove_point:
	...
	insert_row	POINT_ID, DEFAULT_POINT, 0, 1, 0	;just an id selector
	insert_row	REMOVE_POINT, " ", 1, 0, 0	;and a button
	mov	byte[update], 1
	ret
.update:
...
f_new_face:
	...
	mov	byte[error_space], 12
	insert_row	FACE_1, DEFAULT_POINT, 0, 1, 0	;three id selectors
	insert_row	FACE_2, DEFAULT_POINT, 1, 1, 1
	insert_row	FACE_3, DEFAULT_POINT, 2, 1, 2
	insert_row	ADD_FACE, " ", 3, 0, 0	;and a button
	mov	byte[update], 1
	ret
.update:
...
f_new_point:
	...
	insert_row	POINT_X, DEFAULT_STRING, 0, 2, 0	;3 positioncounters
	insert_row	POINT_Y, DEFAULT_STRING, 1, 2, 1
	insert_row	POINT_Z, DEFAULT_STRING, 2, 2, 2
	insert_row	RELATIVE_POINT, DEFAULT_POINT, 3, 1, 0	;and 1 id
	insert_row	INSERT_POINT, " ", 4, 0, 0	;and final button thing
	mov	byte[update], 1
	ret
.apply:
	mov	rbx, 1
